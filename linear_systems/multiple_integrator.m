...
% Systems in question are a N integrator
N = 3;
A1 = zeros(N,N);
A1(1,N) = 1;
B1 = zeros(N,1);
...
dt = 0.1;
Ns = 500;
x0 = zeros(N+1,1);  % initial conditions
x0(1) = 0.2; 
...
for i = 1:(Ns-1)
    ...
    u1 = replace(interface,[u;x;ic],[u2(:,i);x1_sim(:,i);x2_sim(:,i);value(ic)]);
...
for i = 1:Ns
    V_sim(:,i)= replace(V,[vc;x;ic],[value(vc);x1_sim(:,i);x2_sim(:,i);value(ic)]);
    Vdot_sim(:,i)= replace(Vdot,[vc;x;u;ic],[value(vc);x1_sim(:,i);x2_sim(:,i);u2(:,i);value(ic)]);
    alpha1_sim(:,i) = replace(alpha1,[x],[x1_sim(:,i);x2_sim(:,i)]);
    alpha2_sim(:,i) = replace(alpha2,[a2c;x],[value(a2c);x1_sim(:,i);x2_sim(:,i)]);
    alpha3_sim(:,i) = replace(alpha3,[a3c;x],[value(a3c);x1_sim(:,i);x2_sim(:,i)]);
    alpha4_sim(:,i) = replace(alpha4,[a4c;x;u],[value(a4c);x1_sim(:,i);x2_sim(:,i);u2(:,i)]);
end
